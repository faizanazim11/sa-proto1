...
from scripts.config import PROJECT_NAME, PathToStorage, Services
...
config = read_configuration("scripts/constants/logger_conf.yml")

logging_config = config["logger"]
logging_config["level"] = Services.LOG_LEVEL
...
def get_logger():
    ...
    for each_handler in logging_config["handlers"]:
        if each_handler["type"] in ["RotatingFileHandler"] and Services.ENABLE_FILE_LOG:
        ...
        elif each_handler["type"] in ["StreamHandler"] and Services.ENABLE_CONSOLE_LOG:
